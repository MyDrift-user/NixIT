{
  ...
  in {
    nixosConfigurations = {
      ...
      "svgwdc-head-01" = mkAppServer { name = "svgwdc-head-01"; services = [ ./modules/services/headscale  { nixit.ipv4 = "10.20.10.15/24"; nixit.gateway = "10.20.10.1"; nixit.headscale = { publicHost = "access.wdc.gmbh"; baseDomain = "ts.wdc.gmbh"; oidcClientId = "headscale-wdc"; oidcSecretKey = "headscale/oidc-client-secret-wdc"; subnetRouter.routes = [ "10.20.10.11/32" "10.20.10.12/32" "10.20.10.13/32" ];   # DCs + print server only, no whole VLAN }; nixit.pangolin.resources = [{ key = "headscale-wdc"; name = "WDC Access (Headscale)"; fullDomain = "access.wdc.gmbh"; port = 8080; sso = false; healthPath = "/health"; }]; } ]; };  # WDC headscale (control plane only through Pangolin; devices peer over WireGuard; host is subnet router for VLAN 110)
