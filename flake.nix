{
  ...
  in {
    nixosConfigurations = {
      ...
      "svgmdl-outl-01" = mkAppServer { name = "svgmdl-outl-01"; services = [ ./modules/services/outline    { nixit.ipv4 = "10.10.20.21/24"; nixit.pangolin.resources = [{ key = "outline-docs"; name = "Outline"; fullDomain = "docs.lua.li"; port = 3000; sso = false; healthPath = "/_health"; }]; } ]; };  # outline 1
      "svgmdl-outl-02" = mkAppServer { name = "svgmdl-outl-02"; services = [ ./modules/services/outline    { nixit.ipv4 = "10.10.20.22/24"; nixit.outlineUrl = "https://idpa.mydrift.dev"; nixit.pangolin.resources = [{ key = "outline-idpa"; name = "Outline IDPA"; fullDomain = "idpa.mydrift.dev"; port = 3000; sso = false; healthPath = "/_health"; }]; } ]; };  # outline 2 → idpa.mydrift.dev (IDPA)
      ...
      "svgwdc-head-01" = mkAppServer { name = "svgwdc-head-01"; services = [ ./modules/services/headscale  { nixit.ipv4 = "10.20.10.15/24"; nixit.gateway = "10.20.10.1"; nixit.headscale = { publicHost = "access.wdc.gmbh"; baseDomain = "ts.wdc.gmbh"; oidcClientId = "headscale-wdc"; oidcSecretKey = "headscale/oidc-client-secret-wdc"; subnetRouter.routes = [ "10.20.10.11/32" "10.20.10.12/32" "10.20.10.13/32" "192.168.17.10/32" "192.168.17.11/32" "192.168.17.35/32" "192.168.17.36/32" "192.168.17.37/32" "192.168.17.38/32" "192.168.17.39/32" "192.168.17.40/32" "192.168.17.42/32" ]; }; nixit.pangolin.resources = [{ key = "headscale-wdc"; name = "WDC Access (Headscale)"; fullDomain = "access.wdc.gmbh"; port = 8080; sso = false; healthPath = "/health"; }]; } ]; };  # WDC headscale (control plane only through Pangolin; devices peer over WireGuard; host is subnet router for VLAN 110)
      ...
      "svgmdl-eoff-01" = mkAppServer { name = "svgmdl-eoff-01"; services = [ ./modules/services/euro-office { nixit.ipv4 = "10.10.20.24/24"; } ]; };  # euro-office document engine (edit.lua.li) — Rumi document preview/edit
      "svgmdl-vwrd-01" = mkAppServer { name = "svgmdl-vwrd-01"; services = [ ./modules/services/vaultwarden { nixit.ipv4 = "10.10.20.25/24"; } ]; };  # vaultwarden password server (vault.lua.li)
    ...
    deploy.nodes = {
      ...
      "svgmdl-vwrd-01" = mkNode "svgmdl-vwrd-01";
